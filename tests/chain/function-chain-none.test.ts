...
import { BasicTriggerChain, guard } from '../../src';

describe('BasicTriggerChain with responseType none', () => {
  ...
  it('should call handler with chainData', async () => {
    ...
    const chain = new BasicTriggerChain<typeof triggerData>({ responseType: 'none' });
  ...
  it('should throw Error when guard fails', async () => {
    ...
    const chain = new BasicTriggerChain<typeof triggerData>({ responseType: 'none' }).useGuard(failingGuard);
    const handler = chain.handle(handlerFn);
  ...
  it('should execute guards with chainData pattern', async () => {
    ...
    const chain = new BasicTriggerChain<typeof triggerData>({ responseType: 'none' }).useGuard(guard(guardCheck));
    const handler = chain.handle(handlerFn);
    await handler(triggerData, mockContext);
