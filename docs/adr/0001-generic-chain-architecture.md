...
We're refactoring to a generic `RegularChain<TTriggerData, TResponseType>` base. `TTriggerData` is the data from any Azure Functions trigger. `TResponseType` (`'http' | 'none'`) controls whether guard failures return HTTP responses or throw `ChainGuardError`, and whether the handler returns `HttpResponseInit` or `void`. Specialized chains (`HttpChain`, `ValidatedChain`) live in `src/chain/specialized/` and extend the generic base for trigger-specific ergonomics.
