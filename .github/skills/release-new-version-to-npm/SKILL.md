...
- The package version is changed in `package.json`
- `npm install` is used to refresh `package-lock.json`
- The publishable package is built into `dist/`
- The build copies `README.md`, `LICENSE`, and `package.json` into `dist/`
...
3. Decide how to publish.
   - Preferred path: create a GitHub Release and let `.github/workflows/publish-func.yml` publish `./dist`
...
3. Update version files.
   ...
   - Update `package.json` to the confirmed version
...
6. Commit the release preparation.
   ...
   - Commit all the untracked files that were generated during the process, including `package-lock.json`.
...
8. Publish through GitHub Release.
   ...
   - The workflow in `.github/workflows/publish-func.yml` publishes `./dist` on `release.created`
...
- `package.json`, `package-lock.json`, and `dist/package.json` all reflect the intended version
...
- This repo currently publishes from `dist/`, not from the repository root
- `dist/` is gitignored and is not pushed to the remote branch
- The release workflow uses Node 20 and `npm ci`
...
- If `dist/package.json` does not match the root version after build, investigate the build pipeline instead of patching the file manually
